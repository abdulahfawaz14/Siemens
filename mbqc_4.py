...
model, qubits, thetas = MBQL.square_4_z()
...
rmIndicesTrain=np.where(y_train==1) # remove class 2
...
rmIndicesTest=np.where(y_test==1)
...
for i in range(iters):
    ...
    if i % 100 ==0:
        ...
        print(test_accuracy)
        print(model.eval(feed_dict={qubits[0]: batch[0][:,0], 
                              qubits[1]: batch[0][:,1], 
                              qubits[2]: batch[0][:,2], 
                              qubits[3]: batch[0][:,3],
                              y_truth: batch[1]}))

...
model, qubits, thetas = MBQL.square_9_2()
  
def get_batch(bsize):
    batch = [0]*2
    r=np.random.randint(0,y_train.shape[0])
    batch[0] = np.squeeze(np.array([[np.cos(x_train[r])], [np.sin(x_train[r])]]).astype('complex64'))
...
def get_test_accuracy():
    ...
    for i in range(y_test.shape[0]):
        batch = [0]*2
        r=i
        batch[0] = np.squeeze(np.array([[np.cos(x_test[r])], [np.sin(x_test[r])]]).astype('complex64'))
...
buildDataFromIris()
# Reading in the data
data = genfromtxt('directions_train.csv',delimiter=',')  # Training data
test_data = genfromtxt('directions_test.csv',delimiter=',')  # Test data

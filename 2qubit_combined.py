...
model, qubits, thetas = blocks.mps_2_online_mixed()
...
def get_batch(bsize):
    batch = [0]*2
    r=np.random.randint(0,y_train.shape[0])
    batch[0] = np.squeeze(np.array([[np.cos(x_train[r][:2])], 
         [np.sin(x_train[r][:2]) * np.exp(1J*x_train[r][2:])]]))
...
def get_test_accuracy():
    ...
    for i in range(y_test.shape[0]):
        batch = [0]*2
        r=i
        batch[0] = np.squeeze(np.array([[np.cos(x_test[r][:2])], 
         [np.sin(x_test[r][:2]) * np.exp(1J*x_test[r][2:])]]))
